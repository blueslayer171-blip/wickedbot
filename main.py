...
RECORD_CHANNEL_ID = 1500132824861118644

MAPS = ['Factory', 'Skyscraper', 'Hideout', 'Ship', 'Arctic', 'Dam', 'Mall']

def is_manager():
    async def predicate(interaction: discord.Interaction):
        role = discord.utils.get(interaction.user.roles, id=1493171545852346418)
        if role is None:
...
async def update_record(client, scrim_won: bool, map_results: list):
    ...
    async for message in channel.history(limit=50):
        if message.author == client.user and 'LYNX RECORD' in message.content:
...
async def ping_available(interaction: discord.Interaction, day: app_commands.Choice[str]):
    ...
    channel = interaction.client.get_channel(1500132998148784230)
    if not channel:
...
async def clear_record(interaction: discord.Interaction):
    ...
    async for message in channel.history(limit=50):
        if message.author == interaction.client.user and 'LYNX RECORD' in message.content:
...
@bot.event
async def on_ready():
    guild = discord.Object(id=1493171094490714112)
